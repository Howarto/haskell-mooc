...
member elem ( Set ( x : xs ) ) = elem == x || member elem ( Set xs )

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add elem set =
  let
    add' el ( Set [] ) acc = Set $ acc ++ [el]
    add' el (Set y@(x : xs)) acc
      | el < x = Set $ acc ++ [el] ++ [x] ++ xs
      | otherwise = add' el (Set xs) (acc ++ [x])
  in
    if member elem set then set
    else add' elem set []
...
average ( x :| xs ) = average' x 1 xs
  where
    average' acc nItems [] = acc / nItems
    average' acc nItems ( x : xs ) = average' ( acc + x ) ( nItems + 1 ) xs
...
reverseNonEmpty ( x :| xs ) =
  let
    reverseNonEmpty' [] acc = head acc :| tail acc
    reverseNonEmpty' ( x : xs ) acc = reverseNonEmpty' xs ( x : acc )
  in
    reverseNonEmpty' xs [x]
...
passwordAllowed pass ( MinimumLength min ) = length pass >= min
passwordAllowed pass ( ContainsSome str ) = foldr ( \x acc -> acc || elem x str ) False pass
passwordAllowed pass ( DoesNotContain str ) = not $ passwordAllowed pass ( ContainsSome str )
passwordAllowed pass ( And x y ) = passwordAllowed pass x && passwordAllowed pass y
passwordAllowed pass ( Or x y ) = passwordAllowed pass x || passwordAllowed pass y
...
data Arithmetic = Literal Integer String
  deriving Show

literal :: Integer -> Arithmetic
literal int = Literal int ( show int )

operation :: String -> Arithmetic -> Arithmetic -> Arithmetic
operation "+" ( Literal a b ) ( Literal c d ) = Literal ( a + c ) ( "(" ++ b ++ "+" ++ d ++ ")" )
operation "*" ( Literal a b ) ( Literal c d ) = Literal ( a * c ) ( "(" ++ b ++ "*" ++ d ++ ")" )
operation _ _ _ = Literal 0 ( show 0 )

evaluate :: Arithmetic -> Integer
evaluate ( Literal int str ) = int

render :: Arithmetic -> String
render ( Literal int str ) = str
