...
mapMaybe f ( Just str ) = Just ( f str )
...
mapMaybe2 f ( Just x ) ( Just str ) = Just( f x str )
...
firstHalf str = take ( div ( length str ) 2 + mod ( length str ) 2 ) str
...
capitalize xs = unwords . map (\x -> capitalizeFirst x ) . words $ xs

capitalizeFirst :: String -> String
capitalizeFirst str = [ toUpper ( str!!0 ) ] ++ drop 1 str
...
powers :: Int -> Int -> [Int]
powers k max = powers' k max []

powers' :: Int -> Int -> [Int] -> [Int]
powers' k max acc = if current > max then acc
                    else powers' k max ( acc ++ [ current ] )
                    where current = k^( length acc )
...
while check update value = if not . check $ value then value
...
joinToLength l xs = [ result | x <- xs, y <- xs, let result = x ++ y, length result == l ]
...
sumRights' xs i = if i < length xs
                  then
                     case xs!!i of
                         Right ( x ) -> x + sumRights' xs ( i + 1 )
                         _ -> 0 + sumRights' xs ( i + 1 )
                  else 0
...
multiCompose' :: [( a -> a )] -> a -> Int -> a
multiCompose' fs acc 0 = fs!!0 $ acc
multiCompose' fs acc i = multiCompose' fs ( ( fs!!i ) acc ) ( i - 1 )
...
multiApp f fs x = f $ multiApp' fs x
    where
        multiApp' [] _ = []
        multiApp' (f:fs) x = f x : multiApp' fs x
...
interpreter commands = interpreter' commands 0 0 (0, 0) []
    where
        ...
        interpreter' ("printX" : cs) x y (x1, y1) r = interpreter' cs x y (x1, y1) (r ++ [show x1])
        interpreter' ("printY" : cs) x y (x1, y1) r = interpreter' cs x y (x1, y1) (r ++ [show y1])
        interpreter' (c : cs) x y z r = interpreter' cs x y (processCommand c z) r
        processCommand "up" (x, y) = (x, y + 1)
        processCommand "right" (x, y) = (x + 1, y)
        processCommand "down" (x, y) = (x, y - 1)
        processCommand "left" (x, y) = (x - 1, y)
