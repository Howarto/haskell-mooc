...
setName newName (Person age name) = Person age newName

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge newAge (Person age name) = Person newAge name
...
rgb (Mix x y) = mix (rgb x) (rgb y)
  ...
  mix [] _ = []
  mix _ [] = []
  mix (x:xs) (y:ys) = avg x y : mix xs ys
...
toNat :: Int -> Maybe Nat
toNat x = if x < 0 then Nothing else Just( toNat' x )
  where
    toNat' 0 = Zero
...
prettyPrint bin = reverse . prettyPrint' $ bin
  where
    prettyPrint' End = []
...
fromBin :: Bin -> Int
fromBin b = fromBin' 0 b
  where
    fromBin' _ End = 0
    fromBin' pow (O b) = fromBin' (pow + 1) b
    fromBin' pow (I b) = 2 ^ pow + fromBin' (pow + 1) b
