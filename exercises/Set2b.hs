...
oddFactorial 0 = 1
oddFactorial n = case n `mod` 2 of
                 0 -> oddFactorial ( n - 1 )
                 _ -> n * oddFactorial ( n - 1 )
...
leftpad str reqLength = if length str < reqLength
                        then leftpad ( " " ++ str ) reqLength
                        else str
...
countdown number = "Ready! " ++ countdown' number ++ " Liftoff!"

countdown' :: Integer -> String
countdown' number = if number > 0
                    then show number ++ "... " ++ countdown' ( number - 1 )
                    else ""
...
smallestDivisor num = smallestDivisor' num 2

smallestDivisor' :: Integer -> Integer -> Integer
smallestDivisor' num candidate = if num `mod` candidate == 0
                                 then candidate
                                 else
                                 smallestDivisor' num ( candidate + 1 )
