...
longest xs = longest' xs []
    where
        longest' [] acc = acc
        longest' (x:xs) acc = if length x > length acc
                              then longest' xs x
                              else if length x < length acc then longest' xs acc
                              else if head acc <= head x then longest' xs acc
                              else longest' xs x
...
incrementKey k [] = []
incrementKey k ((x, y):xs) = if k == x then (x, y + 1) : incrementKey k xs
                             else (x, y) : incrementKey k xs
...
average xs = average' xs (length xs) 0
    where
        average' [] length acc = acc / fromIntegral length
        average' (x:xs) length acc = average' xs length (acc + x)
------------------------------------------------------------------------------
...
winner scores player1 player2 = if score scores player1 >= score scores player2
                                then player1
                                else player2
                                where
                                    score scores player = Map.findWithDefault 0 player scores
...
freqs xs = foldr (Map.alter f) Map.empty xs
    where
        f Nothing = Just 1
        f (Just x) = Just (x + 1)
...
transfer from to amount bank =
    let
        ...
        updateFrom = Map.update (\x -> Just (x - amount)) from
        updateTo = Map.update (\x -> Just (x + amount)) to
        process Nothing _ _ = bank
        process _ Nothing _ = bank
        process (Just x) (Just y) amount
            | amount <= 0 = bank
            | x - amount < 0 = bank
            | otherwise = updateTo . updateFrom $ bank
    in
...
maxIndex arr = maxIndex' (Data.Array.assocs arr) (head . Data.Array.indices $ arr) (head . Data.Array.elems $ arr)
    where
        maxIndex' [] maxIndex _ = maxIndex
        maxIndex' ((i, v) : xs) maxIndex acc = if v > acc then maxIndex' xs i v
                                               else maxIndex' xs maxIndex acc
