...
maxHelper x acc = if x > acc then x
                  else acc
...
slHelper x (xAcc, yAcc) = (xAcc + x, yAcc + 1)
...
largestHelper x [] = [x]
largestHelper x acc@(accX:accY) = if x > accX then [x]
                                  else if x == accX then acc ++ [x]
                                  else acc
